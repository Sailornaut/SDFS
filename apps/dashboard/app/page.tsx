...
export default async function Dashboard() {
  const [me, approvals, agents, policies, events] = await Promise.all([
    sdfs<Me>("/v1/me"), sdfs<Approval[]>("/v1/approval-requests"), sdfs<Agent[]>("/v1/agents"),
    sdfs<Policy[]>("/v1/policies"), sdfs<Event[]>("/v1/audit")
  ]);
  ...
  return <main className="dashboard">
    ...
    <section className="metrics">
      ...
      <article><span>Decisions</span><strong>{approvals.filter(a => ["APPROVED", "REJECTED"].includes(a.status)).length}</strong><small>in current history</small></article>
    </section>
