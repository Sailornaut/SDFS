...
import {
  ...
  createGrantSchema, createPolicySchema, createPrincipalSchema,
...
app.post("/v1/principals", async (request, reply) => {
  const body = parse(createPrincipalSchema, request.body);
  const principal = await db.principal.create({ data: body });
  await audit("principal.created", "principal", principal.id, "system", "bootstrap", {}, principal.id);
...
app.post("/v1/agents", async (request, reply) => {
  ...
  await audit("agent.created", "agent", agent.id, "principal", body.principalId, {}, body.principalId, agent.id);
...
app.post("/v1/policies", async (request, reply) => {
  ...
  await audit("policy.created", "policy", policy.id, "principal", body.principalId, { effect: body.effect }, body.principalId);
...
app.post("/v1/approval-requests", async (request, reply) => {
  ...
  const approval = await db.approvalRequest.create({ data: {
    ...body, status: evaluation.status, matchedPolicyId: evaluation.matchedPolicyId,
    expiresAt: new Date(Date.now() + body.expiresInSeconds * 1000), expiresInSeconds: undefined
  } });
...
app.get("/v1/approval-requests/:id", async (request) => {
  const { id } = request.params as { id: string };
  return db.approvalRequest.findUniqueOrThrow({ where: { id } });
});

app.post("/v1/approval-requests/:id/decision", async (request, reply) => {
  const { id } = request.params as { id: string };
...
app.post("/v1/capabilities", async (request, reply) => {
  ...
  const capability = await db.capability.create({ data: body });
...
app.get("/v1/audit", async (request) => {
  const { principalId, agentId } = request.query as { principalId?: string; agentId?: string };
  return db.auditEvent.findMany({ where: { principalId, agentId }, orderBy: { occurredAt: "desc" }, take: 200 });
});
